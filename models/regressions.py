...
def evaluate_regression(pipeline: Pipeline, param_grid: dict, X_train, X_test, y_train, y_test, Db_keys):
    ...
    # At each step, we will allow the subset to delete 1 element and add the best pair (allowing to add the deleted element if it is relevant)
    for i in tqdm(range(1, X_train.shape[1])):
        scoring_dict = {}

        for k in best_parameters_ids:
            scoring_dict[k] = {}
            temp_best_parameters_ids = list(
                set(best_parameters_ids) - set([k]))
            temp_exploration = list(
                set(range(X_train.shape[1])) - set(temp_best_parameters_ids))
            ...
            for i, j in list(itertools.combinations(temp_exploration, 2)):
                ...
                X_train_temp = np.take(
                    X_train, temp_best_parameters_ids + [i, j], 1)
                grid_search.fit(X_train_temp, y_train)
                scoring_dict[k].update({i: {j: grid_search.best_score_}})

        [k, i, j, score] = best_parameters_tuple(
            scoring_dict)  # Find best choice of deletion/add

        best_parameters_ids = list(
            set(best_parameters_ids) - set([k])) + [i, j]
    ...
    for id in range(1, len(best_scoring_list)):
        ...
        # Choose the most simple model to have significantly better performances than the previous one.
    ...
    grid_search = GridSearchCV(
        pipeline, {}, cv=5, scoring='neg_mean_squared_error', n_jobs=-1)
    ...
    plt.plot([min(y_test), max(y_test)], [min(y_test), max(y_test)], color='red',
             linestyle='--', label='Perfect Prediction')   # Line for perfect predictions
...
def seek_best_degree(n, X_train, y_train):
    ...
    for i in range(1, n+1):
        pipeline_linreg = Pipeline(
            [('preprocesser', PolynomialFeatures(i)), ('model', SGDRegressor(max_iter=1700))])
        score = evaluate_regression_no_plots(
            pipeline_linreg, {}, X_train, y_train)
...
def evaluate_linear_regression(X, y):
    ...
    evaluate_regression(pipeline_linreg, {}, X_train,
                        X_test, y_train, y_test, X.keys())
...
def evaluate_polynomial_regression(X, y):
    ...
    pipeline_linreg = Pipeline([('preprocesser', PolynomialFeatures(
        degree)), ('model', SGDRegressor(max_iter=1700))])
